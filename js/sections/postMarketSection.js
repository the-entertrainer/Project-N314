...
function _renderSummary() {
  ...
  if (!stocks.length) {
    ...
    if (el) el.innerHTML = '<p class="text-surface-400 text-center py-8">Run the Screener to load market data first.</p>';
    return;
  }
  ...
  const gainersHtml = gainers.map(s => `
    <div class="flex items-center justify-between py-1.5">
      <span class="text-white font-medium text-sm">${s.ticker.replace('.NS', '')}</span>
  ...
  if (el) el.innerHTML = `
    <div class="grid grid-cols-2 gap-4">
      <div class="bg-surface-800 rounded-xl p-4">
        <h4 class="text-xs font-semibold text-green-400 uppercase mb-3">Top 5 Gainers</h4>
        ${gainersHtml}
      </div>
      <div class="bg-surface-800 rounded-xl p-4">
        <h4 class="text-xs font-semibold text-red-400 uppercase mb-3">Top 5 Losers</h4>
        ${losersHtml}
      </div>
    </div>`;
}
...
function _renderLog() {
  ...
  if (!State.postMarketLog.length) {
    container.innerHTML = '<p class="text-surface-500 text-sm text-center py-6">No post-market entries yet. Generate an AI analysis or add notes below.</p>';
    return;
  }
  container.innerHTML = State.postMarketLog.map(e => `
    <div class="bg-surface-800 rounded-xl p-4 border border-surface-700">
      <div class="flex items-center justify-between mb-2">
        <span class="text-xs text-surface-400">${new Date(e.timestamp).toLocaleDateString('en-IN', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
        ${e.source === 'AI' ? '<span class="text-xs bg-purple-900 text-purple-300 px-2 py-0.5 rounded">AI Generated</span>' : ''}
      </div>
      ${e.summary ? `<p class="text-surface-200 text-sm leading-relaxed">${e.summary}</p>` : ''}
      ${e.whatWorked ? `<div class="mt-2"><span class="text-xs font-semibold text-green-400">What Worked: </span><span class="text-sm text-surface-300">${e.whatWorked}</span></div>` : ''}
      ${e.whatFailed ? `<div class="mt-1"><span class="text-xs font-semibold text-red-400">What Failed: </span><span class="text-sm text-surface-300">${e.whatFailed}</span></div>` : ''}
      ${e.watchTomorrow ? `<div class="mt-1"><span class="text-xs font-semibold text-yellow-400">Watch Tomorrow: </span><span class="text-sm text-surface-300">${e.watchTomorrow}</span></div>` : ''}
      ${e.notes ? `<p class="text-surface-400 text-sm mt-2 italic">${e.notes}</p>` : ''}
    </div>`).join('');
}
