    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6" id="strategy-metrics">
      ${['INDIA VIX', 'PCR', 'FII Net', 'Max Pain'].map(l => `
        <div class="bg-surface-800 rounded-xl p-4 text-center">
          <div class="text-xs text-surface-400 mb-1">${l}</div>
          <div class="text-2xl font-bold text-white" id="metric-${l.replace(/\s/g, '-').toLowerCase()}">—</div>
          ${l === 'FII Net' ? '<div class="text-xs text-surface-500">AI Estimated</div>' : ''}
        </div>`).join('')}
    </div>
...
function _renderPivots() {
  ...
  if (!prices.length) {
    document.getElementById('pivot-section').innerHTML = '<p class="text-surface-500 text-sm">Pivot levels unavailable — no Nifty OHLC data.</p>';
    return;
  }
  ...
  document.getElementById('pivot-section').innerHTML = `
    <h3 class="text-sm font-semibold text-surface-300 uppercase mb-3">Pivot Levels</h3>
    <div class="flex flex-wrap gap-2">
      ${['R3', 'R2', 'R1', 'Pivot', 'S1', 'S2', 'S3'].map(k => {
        const val = pivots[k.toLowerCase()];
        const cls = k.startsWith('R') ? 'text-red-400' : k === 'Pivot' ? 'text-yellow-400' : 'text-green-400';
        return `<div class="bg-surface-800 rounded-lg px-4 py-2 text-center">
...
function _renderScenarios(scenarios) {
  ...
  const scenarioHtml = Object.entries(scenarios).map(([key, s]) => {
    ...
    const hourHtml = s.hourByHour ? s.hourByHour.map(h =>
      `<div class="flex gap-3 py-1.5 border-b border-surface-700/50 last:border-0">
        <span class="text-xs font-mono text-surface-400 w-12 flex-shrink-0">${h.time}</span>
        <span class="text-sm text-surface-300 flex-1">${h.action}</span>
        ${h.target ? `<span class="text-xs text-${c}-400 flex-shrink-0">${h.target}</span>` : ''}
      </div>`).join('') : '';

    return `
      <div class="bg-surface-800 rounded-xl border border-${c}-800/50 overflow-hidden">
        <div class="p-4">
          <div class="flex items-center justify-between mb-2">
            <h3 class="font-bold text-white capitalize">${icon} ${key} Scenario</h3>
            <span class="text-lg font-bold text-${c}-400">${s.probability}%</span>
          </div>
          ${s.trigger ? `<p class="text-sm text-surface-300 mb-2"><span class="font-semibold text-surface-200">Trigger:</span> ${s.trigger}</p>` : ''}
          ${s.rationale ? `<p class="text-xs text-surface-400 italic">${s.rationale}</p>` : ''}
          ${key !== 'sideways' ? `
            <div class="flex gap-4 mt-3 text-sm">
              <span><span class="text-surface-400">T1:</span> <span class="text-${c}-400">${s.target1}</span></span>
              <span><span class="text-surface-400">T2:</span> <span class="text-${c}-400">${s.target2}</span></span>
              <span><span class="text-surface-400">SL:</span> <span class="text-red-400">${s.stopLoss}</span></span>
            </div>` : `<p class="text-sm text-yellow-400 mt-2">Range: ${s.range}</p>`}
          ${hourHtml ? `<details class="mt-3"><summary class="text-xs text-surface-400 cursor-pointer hover:text-white">Hour-by-hour plan</summary><div class="mt-2">${hourHtml}</div></details>` : ''}
        </div>
      </div>`;
  });

  container.innerHTML = `
    <h3 class="text-sm font-semibold text-surface-300 uppercase mb-3">Next-Day Scenarios</h3>
...
function _renderAIStrategy(data) {
  ...
  if (!container) return;
  container.innerHTML = `
    <div class="bg-surface-800 rounded-xl border border-purple-800/30 p-5">
      <div class="flex items-center gap-2 mb-4">
        <span class="text-purple-400 text-lg">🤖</span>
        <h3 class="font-bold text-white">AI Strategy Plan</h3>
        <span class="text-xs bg-purple-900 text-purple-300 px-2 py-0.5 rounded">Gemini</span>
      </div>
      ${data.summary ? `<p class="text-surface-200 mb-4 leading-relaxed">${data.summary}</p>` : ''}
      ${data.primaryScenario ? `<div class="mb-4"><h4 class="text-xs font-semibold text-surface-400 uppercase mb-2">Primary Scenario</h4><p class="text-surface-300 text-sm">${data.primaryScenario}</p></div>` : ''}
      ${(data.tradeSetups || []).length ? `<div class="mb-4"><h4 class="text-xs font-semibold text-surface-400 uppercase mb-2">Trade Setups</h4><ul class="space-y-1">${data.tradeSetups.map(s => `<li class="text-sm text-surface-300 flex gap-2"><span class="text-accent">→</span>${s}</li>`).join('')}</ul></div>` : ''}
      ${(data.riskRules || []).length ? `<div class="mb-4"><h4 class="text-xs font-semibold text-red-400 uppercase mb-2">Risk Rules</h4><ul class="space-y-1">${data.riskRules.map(r => `<li class="text-sm text-surface-300 flex gap-2"><span class="text-red-400">⚠</span>${r}</li>`).join('')}</ul></div>` : ''}
      ${data.keyLevels ? `<div><h4 class="text-xs font-semibold text-surface-400 uppercase mb-2">Key Levels</h4><p class="text-sm text-yellow-400">${data.keyLevels}</p></div>` : ''}
    </div>`;
}
